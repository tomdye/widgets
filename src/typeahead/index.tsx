...
export const Typeahead = factory(function Typeahead({
	...
	const { options, getResource, isLoading, meta, find, getOrRead } = resource();
	...
	function onKeyDown(
		...
		const metaInfo = meta(options()) || icache.get('meta');
	...
	const currentItems = getOrRead(options()).flat();
	const isCurrentlyLoading = isLoading(options());
